...
def fetch_usage_from_promql(window=AGGREGATION_WINDOW, time_param=None):
    ...
    results = _promql_query(
        f'sum by ("user.email")(sum_over_time({{"claude_code.token.usage"}}[{window}s]))', time_param
    )

    # Delta token type AND model breakdown per user (for cost calculation)
    type_model_results = _promql_query(
        f'sum by ("user.email", type, model)(sum_over_time({{"claude_code.token.usage"}}[{window}s]))', time_param
    )

    # Delta token type breakdown per user (without model, for backward compat)
    type_results = _promql_query(
        f'sum by ("user.email", type)(sum_over_time({{"claude_code.token.usage"}}[{window}s]))', time_param
    )
