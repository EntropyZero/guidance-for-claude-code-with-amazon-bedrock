...
def parse_args():
    ...
    # Single source of truth for profile resolution: an explicit --profile wins
    # over AWS_PROFILE. Export it so every downstream consumer — the cache path,
    # the credential-process subprocess, and the collector sidecar — resolves
    # the SAME profile from the environment (precedence: --profile flag >
    # AWS_PROFILE env > "ClaudeCode" default at the read sites).
    if args.profile:
