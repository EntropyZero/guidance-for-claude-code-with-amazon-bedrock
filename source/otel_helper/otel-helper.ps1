...
# then the "ClaudeCode" default. The argument is parsed by hand because this
# script is invoked with Go-style flags (otel-helper.cmd passes the same args
# it gives otel-helper.exe), which PowerShell parameter binding can't map.
# Keep in sync with the Go binary's resolveProfile and otel-helper.sh.
